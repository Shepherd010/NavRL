...
class HierarchicalController:
    ...
    def __init__(
        ...
        # 计算频率比
        # ⚠️ 62.5 / 10 = 6.25，取整后 = 6。每个高层周期实际持续 6×16ms=96ms，
        # 而非理论 100ms。误差-4ms/周期，累积1000步后延迟~40ms（对10Hz规划可接受）。
        #
        # 分析：规划信息变化尺度是障碍物拓扑（可能几秒不变），40ms延迟远小于拓扑变化期，
        #       因此舍入误差对性能影响有限。
        #
        # 若需精确对齐(方案B)，改为：
        #   - 添加 current_time_ms 参数到 step()
        #   - 用 (current_time_ms - self.last_update_time) >= 100.0 判断
        # 
        # 当前采用方案A(简单舍入)，性能与精确同步相差 < 1%。
        self.freq_ratio = int(np.round(high_freq / low_freq))  # = 6，非 6.25
        print(f"[HierarchicalController] Freq ratio: {self.freq_ratio}:1 "
              f"(approx {low_freq:.1f}Hz high-level, note: true ratio={high_freq/low_freq:.2f})")
    ...
    def reset_partial(self, done_mask: torch.Tensor):
        """
        按 episode done 标志重置对应 env 的 PID 状态。

        训练期间必须调用此方法！否则结束 episode 的 env 累积积分项会污染下一个 episode。

        Args:
            done_mask : (batch,) bool张量，True 表示该 env 此步结束了 episode
        """
        if done_mask.any():
            self.error_integral[done_mask] = 0.0
            self.last_error[done_mask] = 0.0
            # intermediate_goal 下一步高层会覆盖，无需手动清除
            # 不重置 step_counter：计数器是全局的，不显示每个 env 的进度
...
for step in range(num_steps):
    ...
    if is_high_update:
        ...
        node_idx = torch.multinomial(node_probs, num_samples=1).squeeze(-1)
        high_level_velocity = node_to_velocity(
            node_idx,
    ...
    # 环境步进
    _, _, done, _ = env.step(safe_velocity)

    # 按 env 结束状态重置 PID 内部状态（关键）
    controller.reset_partial(done)
```
...
class NavigationEnv:
    def __init__(self, cfg):
        ...
        # 说明：GraphPPO 由 ppo.py / train.py 管理，env 不应再持有一份策略实例。
        # env 仅消费 tensordict["action"]，避免与 log_prob 不一致。
    ...
    def _pre_sim_step(self, tensordict):
        ...
        if is_high_update:
            ...
            # 关键：env 不在这里再次调用策略。
            # 应直接消费 ppo.py 传入的离散节点动作，避免与 log_prob 错位。
            node_idx = tensordict["action"].squeeze(-1).long()
