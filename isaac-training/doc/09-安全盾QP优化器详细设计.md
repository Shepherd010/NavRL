...
> ⚠️ **梯度流说明（关键）**：OSQP 是一个非参数化数值求解器，其输出 `v_safe` 对策略参数 θ **不可微**。PPO 通过 **score function 估计量**（REINFORCE）将梯度传回策略网络：
> $$\nabla_\theta J = \mathbb{E}\left[A_t \cdot \nabla_\theta \log \pi_\theta(a_t | s_t)\right]$$
> 其中 $a_t$ 是 GraphTransformer 输出的节点索引，$A_t$ 是经过 SafetyShield 后的安全速度执行后产生的 GAE 优势。梯度不需要也无法穿过 QP 求解器。
...
\mathbf{n}_1^T \mathbf{V}_{o_1} - \alpha h_1 \\
\vdots \\
\mathbf{n}_N^T \mathbf{V}_{o_N} - \alpha h_N \\
-v_{\max} \\
-v_{\max} \\
-v_{\max} \\
0 \\
...
> 其中 $h_i = \|\mathbf{p}_{o_i} - \mathbf{p}_{\text{ego}}\| - r_{\text{safe}} - r_i$，$\alpha = 1.0$（CBF 增益）。距离越远，$h_i$ 越大，下界越负，约束越宽松。
...
class SafetyShieldQP:
    ...
    def _solve_single(
        ...
        for i in range(n_obs):
            ...
            # CBF 距离自适应下界：h_i = dist - safe_distance - radius
            # 当 h_i > 0（安全区）：下界降低，约束宽松（遥远障碍物几乎不限制）
            # 当 h_i < 0（危险区）：下界升高，约束更严（要求主动减速）
            cbf_alpha = 1.0
            h_i = dist - self.safe_distance - radius
            l_i = np.dot(n, vel) - cbf_alpha * h_i
            l_list.append(l_i)
